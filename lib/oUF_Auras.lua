...
Auras.DefaultAuras = defaultAuras
...
local function AddAurasToList(auras)
  local PVP = PvP  -- Always off, for now
...
Auras.UpdateAuraList = function()
  wipe(auraList)
  AddAurasToList(Auras.DefaultAuras)
...
local function debug(...)
  ns.util.print(strjoin(" ", tostringall(...)))
end
...
function filterFuncs.default(self, unit, iconFrame, name, rank, icon, count, debuffType, duration, expirationTime, caster, isStealable, shouldConsolidate, spellID, canApplyAura, isBossAura, isCastByPlayer, value1, value2, value3)
  local v = auraList[spellID]
  return not v or bit_band(v, FILTER_DISABLE) == 0
end


function filterFuncs.player(self, unit, iconFrame, name, rank, icon, count, debuffType, duration, expirationTime, caster, isStealable, shouldConsolidate, spellID, canApplyAura, isBossAura, isCastByPlayer, value1, value2, value3)
  local v = auraList[spellID]
...
function filterFuncs.pet(self, unit, iconFrame, name, rank, icon, count, debuffType, duration, expirationTime, caster, isStealable, shouldConsolidate, spellID, canApplyAura, isBossAura, isCastByPlayer, value1, value2, value3)
  local v = auraList[spellID]
  --debug("CustomAuraFilter", "[unit]", unit, "[caster]", caster, "[name]", name, "[id]", spellID, "[filter]", v, "[vehicle]", caster == "vehicle")
...
function filterFuncs.target(self, unit, iconFrame, name, rank, icon, count, debuffType, duration, expirationTime, caster, isStealable, shouldConsolidate, spellID, canApplyAura, isBossAura, isCastByPlayer, value1, value2, value3)
  local v = auraList[spellID]
  if isBossAura then
...
function filterFuncs.party(self, unit, iconFrame, name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellID, canApplyAura, isBossAura, isCastByPlayer, value1, value2, value3)
  local v = auraList[spellID]
  return v and bit_band(v, FILTER_ON_PLAYER) == 0
end


function filterFuncs.focus(self, unit, iconFrame, name, rank, icon, count, debuffType, duration, expirationTime, caster, isStealable, shouldConsolidate, spellID, canApplyAura, isBossAura, isCastByPlayer, value1, value2, value3)
...
function filterFuncs.targettarget(self, unit, iconFrame, name, rank, icon, count, debuffType, duration, expirationTime, caster, isStealable, shouldConsolidate, spellID, canApplyAura, isBossAura, isCastByPlayer, value1, value2, value3)
...
Auras.CustomFilters = filterFuncs
...
local function FindAuraTimer(button, unit)
  ...
  for i = 1, button:GetNumChildren() do
    ...
    if child.text and (child.icon == button.icon or child.cooldown == button.cd) then
      ...
      child.text:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", -3, 3)
      child.text.SetPoint = noop

      child.text:SetFont(FONT, 18, "THINOUTLINE")
...
function Auras.OnShow(cd)
  
end


function Auras.OnHide(cd)
  
end



function Auras.PostCreateIcon(element, button)
  button.overlay:SetTexture(BORDER)
  button.overlay:SetTexCoord(0, 1, 0, 1)
  button.overlay:ClearAllPoints()
  button.overlay:SetPoint("TOPLEFT", 0, 0)
  button.overlay:SetPoint("BOTTOMRIGHT", 0, 0)

  button.cd:SetReverse(true)
  --button.cd:SetScript("OnHide", Auras.OnHide)
  --button.cd:SetScript("OnShow", Auras.OnShow)
  
  button.icon:SetTexCoord(0.07, 0.93, 0.07, 0.93)

  button.count:SetFont(FONT, 18, "THINOUTLINE")
...
-- PostUpdateIcon
-- 
-- Here we do a few things after an icon is updated by oUF:
--  + Desaturate icon if it doesn't originate from the player.
--  + Add a text element for OmniCC cooldowns
-- 
function Auras.PostUpdateIcon(element, unit, button, index, offset, filter, isDebuff)
  local name, _, texture, count, atype, duration, timeLeft, caster, isStealable, shouldConsolidate, spellID = UnitAura(unit, index, button.filter)
...
function Auras.PostUpdate(self, unit)
